...
import { updateUser, updateClerkUserImage } from "@/lib/actions/user.actions";
...
export const AccountProfile = ({ user, btnTitle }: Props) => {
  ...
  const onSubmit = async (values: z.infer<typeof UserValidation>) => {
    try {
      // 1️⃣ Update database user profile
      await updateUser({
        userId: user.id,
        name: values.name,
        path: pathname,
      ...
      // 2️⃣ Update Clerk profile image if changed
      if (clerkUser && values.profile_photo !== clerkUser.imageUrl) {
        await updateClerkUserImage({
          userId: clerkUser.id,
      ...
      console.log('Error Occurred at Acc Profile: ', error.code, error.message);
